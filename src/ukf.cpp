...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  if(!is_initialized_)
  {
    // initialize with measurement values if the sensor is LIDAR
    if(meas_package.sensor_type_ == meas_package.LASER)
    {
      // reading raw measurements from lidar sensor
      x_(0) = meas_package.raw_measurements_(0);
      x_(1) = meas_package.raw_measurements_(1);

      // set covariance x and y
      P_(0,0) = std_laspx_ * std_laspx_;
      P_(1,1) = std_laspy_ * std_laspy_;
    }
    else if(meas_package.sensor_type_ == MeasurementPackage::RADAR)
    {
      // read & convert measurements coordinates to cartesian
...
void UKF::Prediction(double delta_t) {
  ...
  MatrixXd P_aug = MatrixXd(n_aug_,n_aug_).setZero(); // covariance
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  // Measurements vector intitializing
  ...
  MatrixXd z_sig = H * Xsig_pred_; // Measurement sigma points matrix
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  // Measurement segma points matrix
